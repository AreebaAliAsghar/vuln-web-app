...
| **v2.1.0** | Students who want the reference **plus profile editing** | Everything in v2.0.0 plus **Profile Editing — Display Name + Email**: users can edit their **display name** (shown on the dashboard, falls back to username) and **email** (changes require re-verification via the existing verification flow). The display name is stored in a new `display_name` column (nullable, NULL = use username), and email changes issue a fresh verification token that replaces the old one. A session-gated **Resend** button on the profile page allows re-sending the verification email. The profile page also shows the account's **verification status**. Sixth DB-schema change (1 column on `users`); the feature reuses the existing `verification_service` infrastructure. |
| **v2.2.0** | Students who want the reference **plus delete account** | Everything in v2.1.0 plus **Delete Account**: a **Delete Account** card at the bottom of the `/profile` page allows users to permanently delete their account. Users must confirm their **password** (the authorization), the row is **hard-deleted** from the database, and the session is cleared with a redirect to `/login?deleted=1` showing a flash message "Your account has been deleted." No new database column (deletion removes the row); no email notification; rate-limiting and CSRF protection apply via existing middleware. |

The incremental tags between them (**v0.1.2 – v0.1.7**) each close one additional vulnerability — see the [Bug Fixes](#bug-fixes) table for the version-by-version mapping. The feature-enhancement tags build on top of v1.0.0: **v1.0.1** adds the password strength meter, **v1.0.2** the User Profile Page, **v1.0.3** Continue with Google, **v1.0.4** Email Verification on Signup, **v1.0.5** Account Lockout, **v1.0.6** Email OTP 2FA, **v1.0.7** MFA via Authenticator App (TOTP), **v1.0.8** QR Code Login, **v2.0.0** CAPTCHA on Login, and **v2.1.0** Profile Editing.
...
| GET | `/profile` | Authenticated profile page (view info + change password + 2FA toggle + edit display name/email) | Yes |
| POST | `/profile/password` | Change the logged-in user's password (returns JSON) | Yes |
| POST | `/profile` | Update display name or email for the logged-in user (returns JSON) | Yes |
| POST | `/profile/email/resend` | Re-send the email-change verification link (returns JSON) | Yes |
| POST | `/profile/delete` | Delete the logged-in user's account (requires password confirmation; returns JSON) | Yes |
...
| **v2.1.0** | 10 | Profile Editing — Display Name + Email | Users can edit their **display name** (shown on the dashboard, falls back to username) and **email** (changes require re-verification). Display name is stored in a new nullable `display_name` column (NULL = use username); email changes issue a fresh verification token that replaces the old one. A session-gated **Resend** button on the profile page allows re-sending the verification email. The profile page also shows the account's **verification status**. Reuses the existing `verification_service` infrastructure and the verification-resend pattern. Sixth DB-schema change (1 column on `users`). | **Done** |
| **v2.2.0** | 11 | Delete Account | A **Delete Account** card at the bottom of the `/profile` page allows users to permanently delete their account. Users must confirm their **password** (as authorization), the row is **hard-deleted** from the database, and the session is cleared with a redirect to `/login?deleted=1` showing a flash message "Your account has been deleted." The delete flow is protected by the existing **CSRF** (hidden token) and **rate-limit** middleware (5 POSTs per 60s per IP). **No new database column** (deletion removes the row); **no email notification**; no new dependency. | **Done** |
